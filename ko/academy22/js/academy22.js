...
var putRuby = function () {
    ...
    o.setAttribute("id", "myP");
    var findStr = "(?<!ruby>)(?<!<s>)[一-龥朗-鶴]{1,8}(?!<rt)(?!</s>)";
    var replaceStr = '<s>$&</s>';
    ...
    for (i = 0; i < elementsHanja.length; i++) {
        ...
        var x0 = window.scrollX + elementsHanja[i].getBoundingClientRect().left; // X
        var y0 = window.scrollY + elementsHanja[i].getBoundingClientRect().top - 10;

        var textHanja = elementsHanja[i].innerHTML;
        var r = document.createElement("div");
        r.setAttribute("class", "myR");
        r.style.left = x0 + "px";
        r.style.top = y0 + "px";
        r.style.width = elementsHanja[i].getBoundingClientRect().width + "px";
        if (dict[textHanja]) {
            r.innerHTML = dict[textHanja];
        } else {
            r.remove();
        }
